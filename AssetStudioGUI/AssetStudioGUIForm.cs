...
namespace AssetStudioGUI
{
    partial class AssetStudioGUIForm : Form
    {
        ...
        public AssetStudioGUIForm()
        {
            ...
            InitializeComponent();
            Text = $"Studio v{Application.ProductVersion}-MoYi中文限定版";
        ...
        private async void extractFileToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (openFileDialog1.ShowDialog(this) == DialogResult.OK)
            {
                ...
                if (saveFolderDialog.ShowDialog(this) == DialogResult.OK)
                {
                    ...
                    StatusStripUpdate($"导出了{extractedCount}个文件");
                }
            }
        }
        ...
        private async void BuildAssetStructures()
        {
            ...
            var log = $"加载了{assetsManager.assetsFileList.Count}个文件,包含{assetListView.Items.Count}个可导出资源";
            var m_ObjectsCount = assetsManager.assetsFileList.Sum(x => x.m_Objects.Count);
            var objectsCount = assetsManager.assetsFileList.Sum(x => x.Objects.Count);
            if (m_ObjectsCount != objectsCount)
            {
                log += $",有{m_ObjectsCount - objectsCount}个资源无法读取";
            }
        ...
        private void PreviewTexture2D(AssetItem assetItem, Texture2D m_Texture2D)
        {
            ...
            if (image != null)
            {
                ...
                image.Dispose();
                assetItem.InfoText = $"宽: {m_Texture2D.m_Width}\n高: {m_Texture2D.m_Height}\n格式: {m_Texture2D.m_TextureFormat}";
                ...
                assetItem.InfoText += $"\n各项异性等级: {m_Texture2D.m_TextureSettings.m_Aniso}\nMipmap等级: {m_Texture2D.m_TextureSettings.m_MipBias}";
